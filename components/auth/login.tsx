...
import { createFirebaseGoogleLoginPayload, createFirebaseLoginPayload, ensureFirebaseAuthConfigured, isFirebaseAuthConfigured } from "@/lib/auth/firebase-client"
...
export function LoginForm({ branding = { siteName: "AutoPartsPro", logoUrl: "" } }: { branding?: { siteName: string; logoUrl: string } }) {
  ...
  const [firebaseReady, setFirebaseReady] = useState(isFirebaseAuthConfigured)

  useEffect(() => {
    let mounted = true
    ensureFirebaseAuthConfigured().then((configured) => {
      if (mounted) setFirebaseReady(configured)
    })
    return () => {
      mounted = false
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (!(await ensureFirebaseAuthConfigured())) throw new Error("Google sign-in is not configured for this dashboard.")
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const useFirebaseAuth = await ensureFirebaseAuthConfigured()
      setFirebaseReady(useFirebaseAuth)
      const body = useFirebaseAuth ? await createFirebaseLoginPayload(normalizedEmail, password) : { email: normalizedEmail, password, deviceName: "Supplier dashboard" }
      await submitLoginPayload(useFirebaseAuth ? { ...body, deviceName: "Supplier dashboard" } : body)
    } catch (loginError) {
