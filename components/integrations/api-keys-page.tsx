...
export function SupplierApiKeysPage({ access }: { access?: BusinessAccess }) {
  ...
  async function copyApiKey() {
    ...
    try {
      ...
      showToast({ type: "success", title: "API key copied successfully", message: "Your API key was copied to the clipboard." })
    } catch {
  ...
  async function requestEnterpriseApi() {
    ...
    try {
      ...
      showToast({ type: "error", title: "Request failed", message: error instanceof Error ? error.message : "Unable to request Enterprise API access" })
    } finally {
