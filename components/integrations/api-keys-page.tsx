...
export function SupplierApiKeysPage({ access }: { access?: BusinessAccess }) {
  ...
  return <main className="space-y-6">
    ...
    {apiAccess && !apiAccess.allowed ? <Card className="border-destructive/30"><CardHeader><CardTitle className="text-base">Billing action required</CardTitle><CardDescription>{apiAccess.message}</CardDescription></CardHeader><CardContent><Button asChild><Link href={appRoutes.addOns}>Request API add-on</Link></Button></CardContent></Card> : null}
    ...
    <Dialog open={Boolean(secret)} onOpenChange={(open) => { if (!open) setSecret("") }}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Copy your API key now</DialogTitle><DialogDescription>This key will not be shown again. Save it in your backend/server environment.</DialogDescription></DialogHeader><div className="break-all rounded-lg border border-border bg-muted p-4 font-mono text-sm">{secret}</div><DialogFooter><Button type="button" onClick={() => void navigator.clipboard?.writeText(secret)}><Copy className="mr-2 h-4 w-4" />Copy key</Button></DialogFooter></DialogContent></Dialog>
