...
export function SupplierStaffPage({ access, membersPayload, rolesPayload }: { access: BusinessAccessEntry | undefined; membersPayload: MembersResponse; rolesPayload: RolesResponse }) {
  ...
  const validateInvite = () => {
    ...
    if (normalizedFirstName.length > 50) return "First name cannot exceed 50 characters."
    if (normalizedLastName.length > 50) return "Last name cannot exceed 50 characters."
    if (!namePattern.test(normalizedFirstName) || !namePattern.test(normalizedLastName)) return "Name can contain only letters, spaces, apostrophes, periods, and hyphens."
  ...
  const submitEdit = async () => {
    ...
    const normalizedFirstName = editingFirstName.trim()
    const normalizedLastName = editingLastName.trim()
    if (!normalizedFirstName) return showMemberError("Staff first name is required.")
    if (!normalizedLastName) return showMemberError("Staff last name is required.")
    if (normalizedFirstName.length > 50) return showMemberError("First name cannot exceed 50 characters.")
    if (normalizedLastName.length > 50) return showMemberError("Last name cannot exceed 50 characters.")
    if (!namePattern.test(normalizedFirstName) || !namePattern.test(normalizedLastName)) return showMemberError("Name can contain only letters, spaces, apostrophes, periods, and hyphens.")
    ...
    try {
      const response = await fetch(`/api/business/members/${encodeURIComponent(editingMember.id)}?businessAccountId=${encodeURIComponent(access.businessAccount.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ firstName: normalizedFirstName, lastName: normalizedLastName, roleIds: Array.from(editingRoles) }) })
      const payload = await response.json().catch(() => null)
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-border bg-card p-4">
        ...
        <Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value.slice(0, 100))} maxLength={100} placeholder="Search staff" className="mt-4 max-w-sm" />
      ...
      <Dialog open={isInviteOpen} onOpenChange={(open) => (open ? setIsInviteOpen(true) : closeInviteDialog())}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Create staff</DialogTitle><DialogDescription>Create a staff account, assign roles, and apply the login method allowed by this plan. Enterprise SSO can be enabled when the client identity provider is configured.</DialogDescription></DialogHeader><form onSubmit={submitInvite} noValidate className="mt-4 space-y-4"><div className="grid gap-4 md:grid-cols-2"><label className="grid gap-2"><span className="text-sm font-medium">First name</span><Input value={firstName} onChange={(event) => setFirstName(normalizeNameInput(event.target.value))} placeholder="First name" maxLength={50} /></label><label className="grid gap-2"><span className="text-sm font-medium">Last name</span><Input value={lastName} onChange={(event) => setLastName(normalizeNameInput(event.target.value))} placeholder="Last name" maxLength={50} /></label></div><label className="grid gap-2"><span className="text-sm font-medium">Email</span><Input type="email" value={email} onChange={(event) => setEmail(event.target.value.slice(0, 254))} maxLength={254} placeholder="supplier-staff@example.com" /></label><RoleSelector roles={roles} filteredRoles={filteredRoles} selectedRoles={selectedRoles} setSelectedRoles={setSelectedRoles} roleQuery={roleQuery} setRoleQuery={setRoleQuery} toggleRoleSelection={toggleRoleSelection} />{inviteMessage ? <p className="text-sm text-destructive">{inviteMessage}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={closeInviteDialog} disabled={loadingInvite}>Cancel</Button><Button type="submit" disabled={loadingInvite}>{loadingInvite ? "Creating..." : "Create account"}</Button></DialogFooter></form></DialogContent></Dialog>
      ...
      <Dialog open={Boolean(editingMember)} onOpenChange={(open) => !open && setEditingMember(null)}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Edit staff</DialogTitle><DialogDescription>Update the staff name and assigned roles.</DialogDescription></DialogHeader><div className="mt-4 space-y-4"><p className="text-sm text-muted-foreground">{toDisplayName(editingMember?.user.email)}</p><div className="grid gap-4 md:grid-cols-2"><label className="grid gap-2"><span className="text-sm font-medium">First name</span><Input value={editingFirstName} onChange={(event) => setEditingFirstName(normalizeNameInput(event.target.value))} maxLength={50} /></label><label className="grid gap-2"><span className="text-sm font-medium">Last name</span><Input value={editingLastName} onChange={(event) => setEditingLastName(normalizeNameInput(event.target.value))} maxLength={50} /></label></div><RoleSelector roles={roles} filteredRoles={filteredEditRoles} selectedRoles={editingRoles} setSelectedRoles={setEditingRoles} roleQuery={editRoleQuery} setRoleQuery={setEditRoleQuery} toggleRoleSelection={toggleEditRoleSelection} />{memberMessage ? <p className="text-sm text-destructive">{memberMessage}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={() => setEditingMember(null)} disabled={memberLoading}>Cancel</Button><Button type="button" onClick={submitEdit} disabled={memberLoading}>{memberLoading ? "Saving..." : "Save changes"}</Button></DialogFooter></div></DialogContent></Dialog>
