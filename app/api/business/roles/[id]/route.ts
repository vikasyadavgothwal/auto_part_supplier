...
async function proxy(roleId: string, request: NextRequest, method: string) {
  ...
  const query = businessAccountId
    ...
    : ""
  const backend = await requestBackend(`/api/v1/business/roles/${encodeURIComponent(roleId)}${query}`, {
    method,
